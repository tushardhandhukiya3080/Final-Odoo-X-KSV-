...
import {
  ...
  Leaf, Route as RouteIcon, IndianRupee, Users, MapPin, CheckCircle2, Sparkles, Code2,
} from "lucide-react";
...
const RIDE_MODES = [
  { value: "BIKE", label: "Bike", emoji: "🏍️", tint: "bg-pop-pink" },
  { value: "CAR", label: "Car", emoji: "🚗", tint: "bg-pop-cyan" },
];
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative min-h-screen overflow-hidden bg-[#08111f]">
        {/* Layer 1 — animated ocean gradient (CSS stand-in for the WebGL shader) */}
        ...
        <nav className="relative z-10 mx-auto flex max-w-6xl items-center justify-between px-5 py-5">
          <div className="flex items-center gap-2">
            <div className="grid h-10 w-10 place-items-center rounded-xl ring-1 ring-black/10 bg-white text-brand-600 shadow-brutal"><Car className="h-5 w-5" /></div>
          ...
          <div className="flex items-center gap-2">
            <Link href="/login" className="rounded-xl ring-1 ring-black/10 bg-white px-4 py-2 text-sm font-extrabold uppercase text-black shadow-brutal-sm transition-all hover:-translate-y-0.5">Log in</Link>
            <Link href={startHref} className="hidden rounded-xl ring-1 ring-black/10 bg-pop-yellow px-4 py-2 text-sm font-extrabold uppercase text-black shadow-brutal-sm transition-all hover:-translate-y-0.5 sm:inline-block">Get started</Link>
          </div>
        </nav>
        ...
        <div className="relative z-10 mx-auto grid max-w-6xl items-center gap-12 px-5 pb-24 pt-12 md:pt-16 lg:grid-cols-[1.05fr_0.95fr]">
          <div>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <Link href={startHref} className="lp-btn bg-brand-500 text-white">Get started <ArrowRight className="h-4 w-4" /></Link>
              <Link href="/login" className="lp-btn bg-white text-slate-800">Live demo</Link>
            </div>
            <div className="mt-8">
      ...
      {/* ═══════════════ TECH STRIP ═══════════════ */}
      <div className="border-y-2 border-black bg-black py-3">
        <div className="mx-auto flex max-w-6xl flex-wrap items-center justify-center gap-x-6 gap-y-2 px-5 text-xs font-extrabold uppercase tracking-widest text-white/80">
          {["Next.js", "TypeScript", "PostgreSQL", "Prisma", "Razorpay", "WhatsApp", "Leaflet / OSM", "Live SSE"].map((t) => (
      ...
      <section id="features" className="mx-auto max-w-6xl px-5 py-20">
        ...
        <div className="mt-10 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-6">
          ...
          <Feature className="lg:col-span-2" grad="from-indigo-400 to-indigo-600" icon={<Radio className="h-6 w-6" />} title="Live GPS tracking" body="Real live stream — a moving marker + live ETA, active only during the trip." />
          ...
          <Feature className="lg:col-span-6" grad="from-slate-500 to-slate-700" icon={<ShieldCheck className="h-6 w-6" />} title="Multi-tenant RBAC" body="True org isolation with org-scoped sessions. Admins configure; employees ride. No cross-org data leakage — provable live across two seeded organizations." />
        </div>
      </section>
      ...
      {/* ═══════════════ CTA BAND ═══════════════ */}
      <section className="relative overflow-hidden border-y-2 border-black bg-[#08111f]">
        ...
        <div className="relative z-10 mx-auto flex max-w-6xl flex-col items-center px-5 py-20 text-center">
          ...
          <div className="mt-8 flex flex-wrap justify-center gap-3">
            <Link href={startHref} className="lp-btn bg-pop-yellow text-black">Launch the app <ArrowRight className="h-4 w-4" /></Link>
            <Link href="/login" className="lp-btn bg-white text-black">Log in</Link>
          </div>
        </div>
      </section>
      ...
      <footer className="mx-auto max-w-6xl px-5 py-10">
        <div className="flex flex-col items-center justify-between gap-4 sm:flex-row">
          <div className="flex items-center gap-2">
            <div className="grid h-9 w-9 place-items-center rounded-xl ring-1 ring-black/10 bg-brand-500 text-white"><Car className="h-4 w-4" /></div>
          ...
          <a href="#" className="flex items-center gap-1.5 text-xs font-extrabold uppercase text-black/60 hover:text-black"><Code2 className="h-4 w-4" /> Source</a>
        </div>
      </footer>
    </div>
  );
...
function Feature({ grad, icon, title, body, className = "", featured = false }: { grad: string; icon: ReactNode; title: string; body: string; className?: string; featured?: boolean }) {
  return (
  ...
  return (
    <div className="bento bento-hover">
      <div className="flex items-center justify-between">
        ...
        <div className="grid h-10 w-10 place-items-center rounded-xl ring-1 ring-black/10 bg-white shadow-brutal-sm">{icon}</div>
      </div>
...
function Impact({ icon, value, label }: { icon: ReactNode; value: string; label: string }) {
  return (
    <div className="bento-brand text-center">
      <div className="mx-auto grid h-12 w-12 place-items-center rounded-xl ring-1 ring-black/10 bg-white text-brand-600">{icon}</div>
